...
<template>
  <div class="pointer-events-none fixed right-4 top-20 z-50 flex w-[min(420px,95vw)] flex-col gap-2">
    <TransitionGroup name="notify">
      <div v-for="item in items" :key="item.id"
        class="notify-item pointer-events-auto relative overflow-hidden rounded-lg border p-3 shadow"
        ...
        <div class="flex items-start gap-2">
          ...
          <button type="button" class="rounded p-0.5 hover:bg-slate-200/40 dark:hover:bg-slate-700/40"
            @click="notify.remove(item.id)">
            <X class="h-3.5 w-3.5" />
          </button>
        </div>
        <div class="notify-progress absolute bottom-0 left-0 h-0.5 w-full bg-current/40"
